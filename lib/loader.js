'use strict';
module.exports = async function load_demo(url) {
		...
		if(command == "commit")
			...
			while(p.idx < p.txt.length) {
				...
				if(old_running_obj) { // if the object already exists, delete the original
					...
					console.warn(`Deleting non-existent object with already-used ref at ${p.line+1}:${p.idx+1}`);
				}
				if(p.txt[p.idx] == ',') p.idx++;
			}
			...
			while(p.idx < p.txt.length) {
				...
				if(p.txt[p.idx] != ' ') throw new Error(`Expected " " at ${p.line+1}:${p.idx+1}, found ${p.txt[p.idx]} instead.`);
				p.idx++;
				let loc = read_loc(p, running_obj.loc);
				if(p.txt[p.idx] != ' ') throw new Error(`Expected " " at ${p.line+1}:${p.idx+1}, found ${p.txt[p.idx]} instead.`);
				p.idx++;
				let appearance = read_appearance(p, running_obj.appearance);
			...
			timeframe.backward.setmob.push({ckey, ref: old_mob});
	...
	function read_appearance(p, comparison, inherit_overlays = true, x=0, y=0, z=0) {
		...
		if(p.txt[p.idx] == '~') {
			...
			appearance = JSON.parse(JSON.stringify(comparison));
			if(!inherit_overlays) {
				appearance.overlays = [];
		...
		p.idx++;
		if(p.txt[p.idx] != ';') {
			appearance.pixwl_z = read_number(p);
		}
		if(p.txt[p.idx] == '}') { p.idx++; return appearance; }
		if(p.txt[p.idx] != ';') throw new Error(`Expected ; at ${p.line+1}:${p.idx+1}, found ${p.txt[p.idx]} instead`);
		p.idx++;
